...
import { ConfigManager, type Environment, EnvironmentSchema } from '@eduzz/mcp-config';
...
function setup(global = false): void {
...
function configSwitch(name: string): void {
  const configManager = new ConfigManager();
...
function configDelete(name: string): void {
  const configManager = new ConfigManager();
